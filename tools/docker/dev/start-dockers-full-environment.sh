...
docker compose --project-name "${ENVIRONMENT}" -f "${DOCKER_COMPOSE_HOME}/docker compose.yaml" pull
docker compose --project-name "${ENVIRONMENT}" -f "${DOCKER_COMPOSE_HOME}/docker compose.yaml" up -d
