...
class MetricsControllerTest extends BaseControllerTest {
    ...
    void getSuccessRate_whenDateRangeIsNull_thenReturn400() throws Exception {
        ...
        // When
        final ResultActions resultActions = mvc.perform(get(PATH_GET_SUCCESS_RATE, serviceProviderSid)
                                                                .param("hubSid", hubSid.toString())
                                                                .param("paymentType", paymentType.toString())
                                                                .param("movementType", movementType.toString()));
    ...
    void getSuccessRate_whenFiltersApplied_thenReturn200() throws Exception {
        ...
        final PaymentType paymentType = PaymentType.PRE;
        final MovementType movementType = MovementType.DOOR;
        final SuccessRateMetricsResponseInner successRateMetricsResponseInner = restFaker.successRateMetricsResponse().build();

        when(metricsService.getSuccessRate(any(), any(), any(), any(), any())).thenReturn(List.of(successRateMetricsResponseInner));
        ...
        resultActions
                ...
                .andExpect(jsonPath("$[0].date").value(successRateMetricsResponseInner.getDate().toString()))
                .andExpect(jsonPath("$[0].packagesDelivered").value(successRateMetricsResponseInner.getPackagesDelivered()))
                .andExpect(jsonPath("$[0].packagesClosed").value(successRateMetricsResponseInner.getPackagesClosed()))
                .andExpect(jsonPath("$[0].successRate").value(successRateMetricsResponseInner.getSuccessRate()));

        verify(metricsService).getSuccessRate(serviceProviderSid, dateRange, hubSid, paymentType, movementType);
    }
